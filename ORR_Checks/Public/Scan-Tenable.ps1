...
Function Scan-Tenable
{
    ...
    try{
        ...
        $resource = "some_tenable_endpoint"
        $headers.Add("X-ApiKeys", "accessKey=$accessKey; secretKey=$secretKey")
        $onboardingscans = (Invoke-RestMethod -Uri $resource -Method Get -Headers $headers).scans | where {$_.name -like "*some_scan_group*"} | sort name
