...
Function Get-TenableCheck
{
    ...
    # Thsi part can be improved upon, just needed a quick way to write this part for getting v1.0 out the door
    try{
        # grab the agents in the agent group 'some_scan_group' details
        $headers = $null
        $headers = New-Object "System.Collections.Generic.Dictionary[[String],[String]]"
        $resource = "https://cloud.tenable.com/scanners/1/agent-groups/'agent_group_ID'/agents?offset=0&limit=5000"
        ...
        if (($agentinfo.status -ne 'on') -or ($agentinfo.status -ne 'init') -and ($agentinfo.groups.name -notcontains 'some_scan_group'))
        {
